...
@app_commands.describe(message1='This will be displayed on Sir.MilkGobblers computer')
...
async def time_difference(interaction, date1:str, date2:str=None):
    await interaction.response.send_message(discord_time_check(date1, date2))

# bug report command!
@bot.command(
    name ='bug_report',
    description=f'Please use this command to report a bug with {client.user}'
)
@app_commands.describe(
